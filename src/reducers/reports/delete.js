import {DELETE_REPORT_ERROR, DELETE_REPORT_LOADING, DELETE_REPORT_SUCCESS, DELETE_REPORT_RESET} from './constants';

const initialState = {
    loading: false,
    success: false,
    error: null,
    data: null,
    message: null
};

const deleteReportReducer = (state = initialState, action) => {
    const {type, payload} = action;
    switch (type) {
        case DELETE_REPORT_LOADING:
            return {...state, loading: true};
        case DELETE_REPORT_SUCCESS:
            return {
                ...state,
                loading: false,
                success: payload?.success,
                error: null,
                data: payload.data,
                message: payload.message
            };
        case DELETE_REPORT_ERROR:
            return {...state, loading: false, error: payload.error, message: payload.message};
        case DELETE_REPORT_RESET:
            return initialState;
        default:
            return state;
    }
};

export default deleteReportReducer;